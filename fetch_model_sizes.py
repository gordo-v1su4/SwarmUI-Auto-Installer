...
# Import the models structure from the main application
# We need to copy the models_structure here or import it
from Downloader_Gradio_App import models_structure
...
def save_size_data(size_data: Dict, filename: str = "model_sizes.json"):
...
def load_size_data(filename: str = "model_sizes.json") -> Optional[Dict]:
...
def main():
    ...
    if existing_data:
        ...
        response = input("Do you want to re-fetch all sizes? This may take a while. (y/N): ")
        if response.lower() != 'y':
            print("Exiting without re-fetching.")
            return
